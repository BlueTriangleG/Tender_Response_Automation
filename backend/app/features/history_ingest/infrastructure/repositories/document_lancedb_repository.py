"""Persistence adapter for historical document chunks stored in LanceDB."""
...
class DocumentLanceDbRepository:
    """Upsert and lookup document chunks in the configured LanceDB table."""
    ...
    def upsert_records(self, records: list[dict[str, Any]]) -> None:
        """Insert new document chunks and overwrite existing chunks by id."""
    ...
    def get_existing_record_ids(self, record_ids: list[str]) -> set[str]:
        """Return the subset of chunk ids that already exist in the document table."""
