"""XLSX parser for tender-response question imports."""
...
class TenderExcelParser:
    """Extract question rows from the first visible worksheet in an XLSX file."""
    ...
    def parse_bytes(self, raw_bytes: bytes, *, source_file_name: str) -> TenderTabularParseResult:
        """Read workbook bytes and normalize rows from the first visible worksheet."""

        try:
            with ZipFile(BytesIO(raw_bytes)) as workbook_archive:
            ...
            raise ValueError(f"Failed to parse XLSX workbook: {source_file_name}") from exc
